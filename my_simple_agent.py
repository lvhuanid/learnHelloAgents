...
class MySimpleAgent(SimpleAgent):
    ...
    # 1. 先把用户消息记录进历史
            ...
            # 3. 提取纯文本（假设 response 是 LLMResponse，属性为 .content）
            if hasattr(response, 'content'):
                text_response = response.content
            else:
                text_response = str(response)   # fallback

            # 4. 记录助手消息
            self.add_message(Message(text_response, "assistant"))
    ...
    def _run_with_tools(self, messages: list, input_text: str, max_tool_iterations: int, **kwargs) -> str:
        ...
        while current_iteration < max_tool_iterations:
            ...
            # 检查是否有工具调用
            tool_calls = self._parse_tool_calls(response_text)  # 传入字符串

            if tool_calls:
                print(f"🔧 检测到 {len(tool_calls)} 个工具调用")
                tool_results = []
                clean_response = response_text  # 纯文本
